...
public class EnregistrerReductionControleur extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("min1") == "" || request.getParameter("max1") == "" || request.getParameter("reduc1") == "")
		{
			...
			if(Integer.parseInt(request.getParameter("max1")) < (Integer.parseInt(request.getParameter("min1"))))
			{
				String validation = "Le max doit être supérieur au min.";
				request.setAttribute("validation", validation);
			}
			else {
				...
				if(request.getParameter("min2") == "" || request.getParameter("max2") == "" || request.getParameter("reduc2") == "")
				{
					...
					if(Integer.parseInt(request.getParameter("max2")) < (Integer.parseInt(request.getParameter("min2"))) || Integer.parseInt(request.getParameter("min2")) < Integer.parseInt(request.getParameter("max1")))
					{
						String validation1 = "Le max doit être supérieur au min. Si plusieurs lignes sont remplis le max doit être égal ou inférieur au min suivant";
						request.setAttribute("validation", validation1);
					}
					else {
